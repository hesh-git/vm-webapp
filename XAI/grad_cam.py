# Grad-CAM
from .process import get_last_conv_layer
...
def get_grad_cam(model, io_imgs, class_id, LAYER_NAME=None, MODALITY="FLAIR", XAI_MODE="classification", DIMENSION="2d", eps=1e-5):
    ...
    with tf.GradientTape() as tape:
        ...
        predictions = predictions[0]  # only take segmentation prediction
        ...
        grads = tape.gradient(loss, conv_outputs)[0] # tell us which activations in the convolutional layer(s) are most influential in predicting the class_id.
        
        norm_grads = tf.divide(grads, tf.reduce_mean(tf.square(grads)) + tf.constant(eps))
        if DIMENSION=="2d":
            weights = tf.reduce_mean(norm_grads, axis=(0, 1))
        elif DIMENSION=="3d":
            weights = tf.reduce_mean(norm_grads, axis=(0, 1, 2))   # Average gradients spatially
